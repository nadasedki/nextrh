...
export class CvHeuristicParserService {
  ...
  private parseMonthYearToDate(monthStr: string, yearStr: string): Date {
    ...
    return new Date(utcDate - 1 * 60 * 60 * 1000); // Shift for standard local offset representation
  }
  ...
  /**
   * FIX: Added fallback to parse dateless lists of certifications.
   */
  private extractCertifications(section: string) {
    ...
    // FIX: Fallback to parse standard lines as dateless certificates
  ...
  /**
   * FIX: Added Technicien as an academic degree marker.
   */
  ...
  private parseEducationBlock(yearStr: string, lines: string[]) {
    ...
    // Included Technicien inside degree markers
  ...
  /**
   * FIX: Added fallback to parse dateless, bulleted lists of projects (using Wingdings/Symbols).
   */
  private extractProjects(section: string) {
    ...
    // FIX: Parse dateless bulleted list of projects
    if (projects.length === 0) {
      for (const line of rawLines) {
        ...
        if (cleanLine.length > 5) {
          ...
          // Semantic splitter: "pour le compte de"
  ...
  /**
   * FIX: dateStartRegex updated to match lines starting directly with raw years/year ranges (e.g. "2006 - 2016")
   */
  private extractExperiences(section: string) {
    const rawLines = section.split('\n').map(l => l.trim()).filter(l => l.length > 0);
    const combinedLines: string[] = [];
    
    // Expanded regex to capture standalone years/ranges
      ...
      if (match) {
        ...
        if (dateRangeStr.toLowerCase().startsWith('depuis')) {
          ...
          if (parts.length >= 2) {
            ...
            // Check if parts are years only (e.g. 2006) or month/year
