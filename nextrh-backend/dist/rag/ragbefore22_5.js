...
let RagService = class RagService {
    ...
    async ask(question) {
        ...
        const results = await this.vectorService.search(queryVector, targetCandidate, 1);
...
`;
        try {
            ...
            const answer = response.data.message?.content || 'Erreur lors de la génération de la réponse.';
            this.evaluationService.log({
                id: Date.now().toString(),
                question,
                contexts: results.map(r => r.payload.text),
                answer,
                ground_truth: null,
                metadata: {
                    model: 'qwen2.5:7b',
...
//# sourceMappingURL=ragbefore22_5.js.map