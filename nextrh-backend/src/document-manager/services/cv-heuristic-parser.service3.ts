...
export class CvHeuristicParserService {
  ...
  /**
   * ENHANCED: Uses Jaro-Winkler Distance matching to recognize semantic synonyms
   * for section headers, preventing failures on layout changes or different languages.
   */
  private segmentText(text: string): Record<string, string> {
    ...
    for (const line of lines) {
      ...
      // Only check semantic similarity for short heading-like lines
      if (trimmedLine.length < 40) {
        for (const [sectionKey, synonyms] of Object.entries(headingSynonyms)) {
          for (const synonym of synonyms) {
            // Calculate distance metric (value between 0 and 1)
            const score = natural.JaroWinklerDistance(trimmedLine, synonym);
            if (score > 0.88) { // Matches close spellings or near exact strings
  ...
  private parseMonthYearToDate(monthStr: string, yearStr: string): Date {
    ...
    return new Date(utcDate - 1 * 60 * 60 * 1000); // CET offset representation
  }
  ...
  /**
   * ENHANCED: Uses Named Entity Recognition (NER) to isolate Candidate Name and Location
   * dynamically from unstructured streams without being bound to specific lines.
   */
private extractContactInfo(text: string) {
    const lines = text.split('\n').map(l => l.trim()).filter(l => l.length > 0);
    
    let nameIdx = 0;
    // Skip standalone digits (page numbers) or tiny characters
    while (nameIdx < lines.length && (lines[nameIdx].match(/^\d+$/) || lines[nameIdx].length <= 2)) {
      nameIdx++;
    }

    // FIX: Layout-based extraction is highly robust for standard chronological CV headers
    const fullName = lines[nameIdx] || 'Unknown';
    const profession = lines[nameIdx + 1] || '';

    // Prioritize localized regex extractor for the address line 
    ...
    // Fallback to places NER matching ONLY if no prefixed line is found
    if (!address) {
      const doc = nlp(text);
