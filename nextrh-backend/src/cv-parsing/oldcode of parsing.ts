...
// a supprimer
  ...
  async parseEntireCv(rawText: string) {
    const cleanRaw = rawText
      .replace(/[\t]/g, ' ')
      .replace(/\s{3,}/g, '  ') // Crucial pour Anouar : garde les séparateurs de colonnes
      .trim();
   ...
   /* 3. PIPELINE HYBRIDE : Validation et Secours par LLM
    for (const key of Object.keys(result)) {
      if (this.isSectionInvalid(key, result[key])) {
        this.logger.warn(`⚠️ Section [${key}] mal parsée. Tentative de réparation via Qwen...`);
        const repairedData = await this.llmService.repairSection(key, sections[key]);
        if (repairedData) result[key] = repairedData;
      }
    }
