...
let CvHeuristicParserService = CvHeuristicParserService_1 = class CvHeuristicParserService {
    ...
    parse(rawText, cvId = 42, userId = 13, filePath = '') {
    ...
    extractCertifications(section) {
        ...
        if (certifications.length === 0) {
            const joinedCerts = [];
            for (const line of rawLines) {
                if (line.match(/^\s*Certificat[s]?\s+Date\s+d’obtention\s*$/i) || line.match(/^\d+$/))
                    continue;
    ...
    extractEducation(section) {
        ...
        let blockLines = [];
        for (const line of rawLines) {
            if (line.match(/^\s*(Année|Période)\s+Institution\s+Diplôme\s*$/i))
                continue;
...
//# sourceMappingURL=cv-heuristic-parser.service.js.map